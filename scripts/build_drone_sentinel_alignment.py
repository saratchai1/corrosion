...
The output is deliberately a *spatial cross-check*, not a temporal erosion
measurement.  Each Sentinel-2 RGB image is reprojected to the exact geographic
extent and display canvas used by the corresponding drone orthomosaic preview.
A separate SVG, rendered by the web browser, carries the project polygon plus
2025 satellite WATERLINE and MANGROVE_EDGE_PROXY vectors.  No labels or vector
boundaries are baked into either raster.
"""
...
def build_sentinel_canvas(
    ...
    for target_index, band in enumerate(("B4", "B3", "B2")):
        ...
        if path is None or not path.exists():
            raise FileNotFoundError(f"missing downloaded {band}: {path}")
...
def flatten_geometries(collection: dict[str, Any]) -> list[Any]:
    return [shape(feature["geometry"]).buffer(0) for feature in collection.get("features", [])]
...
def svg_path_for_geometry(geom: Any, bounds: list[float], width: int, height: int) -> str:
    ...
    def line_path(coords: Iterable[tuple[float, float]], close: bool = False) -> str:
        points = [xy((float(x), float(y))) for x, y, *_ in coords]
        if not points:
            return ""
...
def write_overlay_svg(
    ...
    *,
    ...
    crop = box(*bounds)
    plot = plot.intersection(crop)
    waterline = waterline.intersection(crop)
    mangrove_edge = mangrove_edge.intersection(crop)
    plot_path = svg_path_for_geometry(plot, bounds, width, height)
    water_path = svg_path_for_geometry(waterline, bounds, width, height)
    mangrove_path = svg_path_for_geometry(mangrove_edge, bounds, width, height)
    centroid = plot.centroid if not plot.is_empty else crop.centroid
    left, bottom, right, top = map(float, bounds)
  ...
  <g fill="none" vector-effect="non-scaling-stroke">
    <path d="{plot_path}" stroke="#ffe35a" stroke-width="3" fill="rgba(255,227,90,0.05)" vector-effect="non-scaling-stroke"/>
...
def main() -> int:
    ...
    waterline = transform_geom(
        shape({
            ...
            "geometries": [mapping.__wrapped__(geom) if False else {} for geom in []],
        }) if False else __import__("shapely").ops.unary_union(flatten_geometries(read_json(resolve(args.waterline)))),
        "EPSG:4326",
        "EPSG:32647",
    )
    mangrove_edge = transform_geom(
        __import__("shapely").ops.unary_union(flatten_geometries(read_json(resolve(args.mangrove_edge)))),
        "EPSG:4326",
        "EPSG:32647",
    )
    ...
    day_gap = (satellite_date - provisional_date).days

    alignments: list[dict[str, Any]] = []
    for plot_row in summary["plots"]:
        plot_id = plot_row["plot_id"]
        raw = by_plot_raw[plot_id]
    ...
    alignment_summary = {
        ...
        "drone_source_date": summary["date_evidence"]["folder_labels"][0],
        ...
        "scientific_guard": (
            "The drone folder date is not verified as the acquisition date. The "
            "Sentinel-2 image is 51 days after that provisional date. This product "
            "is therefore for spatial/geometric validation only, not a same-day "
            "before/after comparison or a drone-derived erosion-rate estimate."
        ),
        "plots": alignments,
    }
    write_path = drone_dir / "sentinel_alignment.json"
    write_path.write_text(json.dumps(alignment_summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    (web_dir / "summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
